...
class RuntimeRagdollBuilder
{
    ...
    ArrayList bones;
    BoneInfo rootBone;
    public void BuildStart(Transform ragdoll)
    {
        //Cleanup();

        pelvis = ragdoll.Find("Root/J_Bip_C_Hips");
        leftHips = pelvis.Find("J_Bip_L_UpperLeg");
        leftKnee = leftHips.Find("J_Bip_L_LowerLeg");
        leftFoot = leftKnee.Find("J_Bip_L_Foot");
        rightHips = pelvis.Find("J_Bip_R_UpperLeg");
        rightKnee = rightHips.Find("J_Bip_R_LowerLeg");
        rightFoot = rightKnee.Find("J_Bip_R_Foot");
        leftArm = pelvis.Find("J_Bip_C_Spine/J_Bip_C_Chest/J_Bip_C_UpperChest/J_Bip_L_Shoulder/J_Bip_L_UpperArm");
        leftElbow = leftArm.Find("J_Bip_L_LowerArm");
        rightArm = pelvis.Find("J_Bip_C_Spine/J_Bip_C_Chest/J_Bip_C_UpperChest/J_Bip_R_Shoulder/J_Bip_R_UpperArm");
        rightElbow = rightArm.Find("J_Bip_R_LowerArm");
        middleSpine = pelvis.Find("J_Bip_C_Spine");
        head = middleSpine.Find("J_Bip_C_Chest/J_Bip_C_UpperChest/J_Bip_C_Neck/J_Bip_C_Head");
        totalMass = 80;
        //Debug.Log(leftHips.gameObject);
        CheckConsistency();
        CalculateAxes();
        BuildCapsules();
        AddBreastColliders();
    ...
    void PrepareBones()
    {
        ...
        AddMirroredJoint("Hips", leftHips, rightHips, "Pelvis", worldRight, worldForward, -90, 90, 30, typeof(CapsuleCollider), 0.3F, 1.5F);
        AddMirroredJoint("Knee", leftKnee, rightKnee, "Hips", worldRight, worldForward, -130, 0, 0, typeof(CapsuleCollider), 0.25F, 1.5F);

        AddJoint("Middle Spine", middleSpine, "Pelvis", worldRight, worldForward, -20, 20, 10, null, 1, 2.5F);

        AddMirroredJoint("Arm", leftArm, rightArm, "Middle Spine", worldUp, worldForward, -90, 90, 90, typeof(CapsuleCollider), 0.25F, 1.0F);
    ...
    void BuildCapsules()
    {
        foreach (BoneInfo bone in bones)
        {
            ...
            collider.radius = Mathf.Abs(distance * bone.radiusScale * 0.5f);
        }
    }
    ...
    void BuildJoints()
    {
        foreach (BoneInfo bone in bones)
        {
            ...
            if (bone.name.Contains("Right Arm") || bone.name.Contains("Right Elbow"))
                joint.axis *= -1;
